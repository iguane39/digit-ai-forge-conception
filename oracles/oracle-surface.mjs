...
if (surface === null || surface.length === 0) {
  ...
  // S1 — chaque élément non couvert est nommé, un constat par élément.
  for (const [i, s] of surface.entries()) {
    const ou = s?.id ? `surface[${i}] (${s.id})` : `surface[${i}]`
    const libelle = s?.libelle ? ` — ${s.libelle}` : ''
    constats.push(couverts.has(s?.id)
      ? constat('S1', PASS, ou, `couvert${libelle}`)
      : constat('S1', FAIL, ou, `élément de surface sans aucune exigence${libelle}`))
  }
