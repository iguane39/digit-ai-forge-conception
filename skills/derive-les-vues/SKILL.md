...
description: Dérive du référentiel d'exigences les trois vues attendues par les forges aval — la fiche de cadrage 6 champs de Forge Design, la configuration de mission de la SaaS Forge, et l'export d'identifiants consommable par le champ risque de Forge Tests — chacune scellée par DEUX empreintes — celle de sa source, pour qu'une vue périmée soit détectée, et celle de son propre corps, pour qu'une vue amputée le soit aussi. Use when / déclencher dès qu'un référentiel d'exigences existe et qu'il faut le passer à une forge aval, produire une fiche de cadrage design, préparer le cadrage d'une mission SaaS Forge, exporter des identifiants d'exigence pour une campagne de tests, régénérer des vues après modification du référentiel, ou décliner un rétro-modèle en documentations par audience (vues par profil PO / CSM / utilisateur, scellées elles aussi par le double sceau). Ne pas déclencher pour rédiger ou modifier les exigences elles-mêmes (→ redige-les-exigences), ni pour exécuter une forge aval — ce verbe dépose des artefacts, il n'invoque personne.
version: 1.6.0
---
...
en jeu fermé — `po` · `csm` · `utilisateur` — chaque vue scellée par le **même double
sceau** que la première famille et chaque affirmation ancrée `[RM-xxx]`. Le sceau y vit dans
le frontmatter, en deux champs : `source_sha256` (péremption détectée, jugée par VP2) et
`corps_sha256` (contenu, jugé par **VP5**, TF-0827). Même raison qu'en TF-0818, et même
mesure : la section « Règles de gestion » d'une vue PO **vidée de son contenu, le titre laissé
en place** — 161 caractères sur 598 — rendait VP1 à VP4 tous PASS, exit 0. VP4 vérifie une
liste close de sections imposées : elle voit un titre, jamais ce qu'il y a dessous. Jugées par
`node oracles/oracle-vues-profil.mjs <VUE.md> --modele <RETRO-MODELE.md>` (VP1-VP5).
